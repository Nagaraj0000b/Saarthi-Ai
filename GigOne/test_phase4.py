...
def run_test():
    ...
    # --- TURN 2: MOOD ---
    print("\n--- TURN 2: MOOD ---")
    state["user_input"] = "I am feeling a bit tired but okay"
    state = graph.invoke(state)
    ...
    # --- TURN 3: PLATFORM ---
    print("\n--- TURN 3: PLATFORM ---")
    state["user_input"] = "I worked on Uber today"
    state = graph.invoke(state)
    print(f"👤 User: {state.get('user_input')}")
    ...
    # --- TURN 4: INVALID PLATFORM (RETRY) ---
    print("\n--- TURN 4: INVALID PLATFORM (RETRY) ---")
    # Reset step to platform_selection to test retry
    state["current_step"] = "platform_selection"
    state["user_input"] = "I worked on something else"
    state = graph.invoke(state)
    print(f"👤 User: {state.get('user_input')}")
    print(f"🤖 AI: {state.get('final_summary')}")
    print(f"📍 Step: {state.get('current_step')}")
